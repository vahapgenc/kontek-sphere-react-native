// ============================================================
// data.ts — mock fixtures for the prototype
// Ported faithfully from design/SPHERE - Full Prototype/k-data.jsx
...
export const UPCOMING_SICK2: Upcoming = {
  month: 'June 2026',
  payday: 'June 25, 2026',
  estNet: 23270,
  prevNet: 25120,
...
export const TODOS_SICK2: Todo[] = []; // 2 days < 7 → no certificate, nothing to do
export const ACTIVITY_SICK2: ActivityItem[] = [
...
function _fmtDay(iso: string): HistoryEntry {
...
function genDays(pool: string[], n: number, offset?: number): HistoryEntry[] {
  if (!n || n <= 0) return [];
  const out: HistoryEntry[] = [];
  const start = (offset || 0) % pool.length;
  for (let i = 0; i < n; i++) out.push(_fmtDay(pool[(start + i) % pool.length]));
  return out;
}
function genLeave(offset?: number): HistoryEntry[] {
  const s = (offset || 0) % LEAVE_POOL.length;
  return [LEAVE_POOL[s], LEAVE_POOL[(s + 1) % LEAVE_POOL.length]];
}

/** A balance before its history has been generated. */
type BalanceSeed = Omit<Balance, 'history'>;

function withHistory(balArr: BalanceSeed[], offset?: number, sickOverride?: HistoryEntry[]): Balance[] {
  return balArr.map(function (b): Balance {
...
const TOBIAS_SICK: HistoryEntry[] = [
...
// Absence balances
export const BALANCES: Balance[] = withHistory([
...
const mkBal = (sem: number, vab: number, sick: number, offset?: number, sickHist?: HistoryEntry[]): Balance[] => withHistory([
