...
function ExpenseFlow({ nav, store }) {
  ...
  const [category, setCategory] = useStateEx("");
  ...
  const [catOpen, setCatOpen] = useStateEx(false);
  const cat = EXP_CATS.find(c => c.id === category);
  ...
  function submit() {
    store.addActivity({
      ...
      title: cat ? `Expense – ${cat.label}` : "Expense",
    ...
    if (!receipt) {
      store.todos.push({ id: "et" + Date.now(), kind: "expense", title: "Expense missing receipt", sub: `Expense · SEK ${amt}`, status: "action", cta: "Attach receipt" });
    }
    setDone(true);
  }
  ...
  if (done) {
    return <Success
      ...
      onSecondary={() => { setStep(0); setAmount(""); setCategory(""); setDesc(""); setReceipt(null); setNoReceipt(false); setDone(false); }}
    />;
  }
  ...
  if (cur === "details") {
    ...
    content = (
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div>
          ...
          <button type="button" onClick={() => setCatOpen(true)} style={{ width: "100%", marginTop: 8, display: "flex", alignItems: "center", gap: 10, border: "1.5px solid var(--line)", borderRadius: "var(--r-md)", padding: "13px 16px", background: "var(--surface)", fontFamily: "var(--font)", cursor: "pointer", textAlign: "left" }}>
            <span style={{ flex: 1, minWidth: 0, fontSize: "var(--t-body-sm)", color: cat ? "var(--ink)" : "var(--ink-3)" }}>{cat ? cat.label : "Choose a category"}</span>
            <Icon name="chevD" size={18} stroke="var(--ink-3)" />
          </button>
        </div>
        <div>
        ...
        <Sheet open={catOpen} onClose={() => setCatOpen(false)} title="Category" sub="Choose what the expense is for">
          <div style={{ padding: "4px 0 8px", display: "flex", flexDirection: "column", gap: 8, maxHeight: "min(56vh, 440px)", overflowY: "auto" }}>
            {EXP_CATS.map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => { setCategory(c.id); setCatOpen(false); }}
                  ...
                  <span style={{ flex: 1, minWidth: 0, fontSize: "var(--t-title)", fontWeight: 700 }}>{c.label}</span>
    ...
    footer = <Button full size="lg" disabled={!amt || !category || (!receipt && !noReceipt)} onClick={next}>{receipt ? "Continue" : "Continue without receipt"}</Button>;
  }
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <Card pad={0}>
          <div style={{ padding: "4px 16px" }}>
            {cat && <SummaryRow label="Category" value={cat.label} />}
...
Object.assign(window, { ExpenseFlow, CompleteFlow });
