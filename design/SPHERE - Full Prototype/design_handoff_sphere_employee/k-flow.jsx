...
function FlowShell({ onClose, onBack, step, total, title, kicker, children, footer }) {
  return (
    <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", background: "var(--bg)" }}>
      ...
      {footer && <div className="ds-cta-bar" style={{ paddingBottom: 24, background: "var(--bg)", borderTop: "none", boxShadow: "none" }}>{footer}</div>}
    </div>
  );
}
...
function OptionCard({ icon, title, desc, selected, onClick, badge, badgeTone }) {
  return (
    <button onClick={onClick} style={{ width: "100%", textAlign: "left", display: "flex", alignItems: "center", gap: 14, padding: 15, fontFamily: "var(--font)", cursor: "pointer",
      ...
      border: selected ? "2px solid var(--signature)" : "1px solid var(--line)",
      boxShadow: selected ? "0 0 0 3px rgba(32,59,60,0.12)" : "var(--shadow-sm)",
...
function Calendar({ year = 2026, month = 5, range = false, value, onChange }) {
  ...
  return (
    <div style={{ background: "var(--surface)", borderRadius: "var(--r-lg)", padding: 16, border: "1px solid var(--line-2)", boxShadow: "var(--sh-1)" }}>
...
function ImpactCard({ amount, lines, payday = PERIOD.payday }) {
  return (
    <div style={{ background: "var(--surface)", borderRadius: "var(--r-lg)", border: "1px solid var(--line-2)", boxShadow: "var(--sh-1)", overflow: "hidden" }}>
...
function Attachment({ label = "Attach receipt", hint, value, onChange, optional }) {
  ...
  return (
    <button onClick={() => onChange(label.includes("certificate") ? "Medical_certificate.pdf" : "Kvitto_" + Date.now().toString().slice(-4) + ".jpg")} style={{ width: "100%", fontFamily: "var(--font)", cursor: "pointer", display: "flex", alignItems: "center", gap: 12, padding: 15,
...
function Success({ title, lines, status = "pending", onPrimary, primaryLabel = "Done", onSecondary, secondaryLabel }) {
  ...
  return (
    <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", background: "var(--bg)" }}>
      ...
      <div style={{ flexShrink: 0, position: "sticky", bottom: 0, padding: "12px 20px 24px", display: "flex", flexDirection: "column", gap: 10, background: "var(--bg)" }}>
