  ...
  return (
    <div className="ds-mcard" style={{ padding: 0, overflow: "hidden" }}>
      <button onClick={toggle} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", border: "none", background: "transparent", cursor: "pointer", fontFamily: "var(--font)", textAlign: "left", borderBottom: open ? "1px solid var(--line-2)" : "none" }}>
        <span style={{ flex: 1, display: "inline-flex", alignItems: "center", gap: 8, whiteSpace: "nowrap", fontSize: "var(--t-caption)", fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", color: "var(--ink-3)" }}>{title}{right}</span>
...
function Row({ icon, iconClass = "", title, sub, right, onClick, last, full, dense, style }) {
  ...
  return (
    <Tag className={"ds-row" + (full ? " ds-row--full" : "")} onClick={onClick} style={{ ...(dense ? { minHeight: 52 } : null), ...style }}>
      {icon && (
        <span className={"ds-row__lead " + iconClass} style={dense ? { width: 34, height: 34 } : undefined}>
      ...
      {right !== undefined ? <span className="ds-row__trail">{right}</span>
        : (onClick && <span className="ds-row__chev"><Icon name="chevR" /></span>)}
    </Tag>
  );
}
